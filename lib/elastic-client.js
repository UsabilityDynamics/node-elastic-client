...
function elasticClient() {
  ...
  // Extract URL parts.
  if( instance.get( 'url' ) ) {
    instance.set( this.utility.parse_url( instance.get( 'url' ) ) );
  }
...
Object.defineProperties( elasticClient.prototype, {
  ...
  getModel: {
     ...
     * @for elasticClient
     */
    value: function getModel( path, callback ) {
  ...
  request: {
    ...
    value: function request( method, path, data ) {
      ...
      this.utility.request({
        url: [ 'http://', this.get( 'host', 'localhost' ), ':', this.get( 'port', 9200 ), '/', path ].join( '' ),
        json: true,
        body: data,
        qs: data,
